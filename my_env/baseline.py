...
from .server.data.constants import DEFAULT_RANDOM_SEED
...
class OpenAIBaselineAgent:
    def __init__(self, client: OpenAI, model: str, temperature: float, seed: int) -> None:
        self._client = client
        self._model = model
...
def main() -> None:
    ...
    if not api_key and fallback_mode != "deterministic":
    ...
    if fallback_mode == "deterministic" and not api_key:
        print("OPENAI_API_KEY not set. Running deterministic fallback baseline.")
        runner = BaselineRunner(agent=DeterministicFallbackAgent())
        run_label = "DeterministicFallback"
    else:
        client = OpenAI(api_key=api_key)
        # Fixed low-temperature, fixed seed for reproducible baseline behavior.
        agent = OpenAIBaselineAgent(client=client, model=model, temperature=0.0, seed=inference_seed)
    ...
    try:
        ...
        if fallback_mode != "deterministic":
            raise RuntimeError(
                "OpenAI API quota exceeded (429 insufficient_quota). "
                "Set OPENAI_BASELINE_FALLBACK=deterministic to run a local fallback baseline, "
                "or add API quota and retry."
            ) from exc

        print("OpenAI quota exceeded. Switching to deterministic fallback baseline.")
        fallback_runner = BaselineRunner(agent=DeterministicFallbackAgent())
        results = [fallback_runner.run_task(task=task, seed=seeds[task]) for task in task_order]
        run_label = "DeterministicFallback"
    ...
    macro_average = sum(result.average_reward for result in results) / float(len(results))
    macro_total = sum(result.total_reward for result in results)
    print(f"macro_avg_reward={macro_average:.6f}")
